...
const Cuentos = () => {
    const { user, loading, cuentos, cuento, resolveCuento, noCuento, deleteCuento, editarCuento, admins, block } = useApp()
    ...
    useEffect(() => {
        const setUsers = () => {
            ...
            cuentos.forEach((cuento) => {
                if (!c.includes(cuento.user) && !block.includes(cuento.user_email)){
                    c.push(cuento.user)
                }
                        ...
                        return (
                            <div className="d-flex justify-content-center" key={i}>
                                <div className="caja-autor">
                                    ...
                                    <button className="btn btn-dark" onClick={(e) => { e.preventDefault(); setUsuario(usuario) }}>Ver Los Textos</button>
                                </div> </div>
                        )
                    }
                    )}
            ...
            if (!cuentoF && !cargando) {
                return (
                    <div>
                        ...
                        <ModalAlertEdit />
                        <div>
                            <h4 className="text-center">Textos de {usuario}</h4>
                            <button onClick={(e) => { e.preventDefault(); setUsuario("") }} className="btn btn-light">Volver</button>
